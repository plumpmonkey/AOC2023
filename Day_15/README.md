...
### Part Two
A much more indepth part 2. Created a `Box` class to store the `box number`, the list of `(lens, focal_length)` and a set of `lens` so we can quickly tell if a lens is in the box without having to loop through the list each time.

With that in place, it was simple to follow the rules for adding, removing or updating the lenses in the boxes.
